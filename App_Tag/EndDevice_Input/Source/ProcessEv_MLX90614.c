...
/* センサーの電源をDO3によるON/OFF式にしたので、以下の変換時間はPower Onからの変換時間を設定する */
/* データシートでは150msとなっていたが、ばらつきが大きいので200msとした。フィルタはデフォルトのまま */
#define MLX90614_WAITTIME 	(200)	  // POWER ON RESETからの待ち時間(ms)

static void vProcessEvCore(tsEvent *pEv, teEvent eEvent, uint32 u32evarg);
static void vStoreSensorValue();
static void vProcessMLX90614(teEvent eEvent);
static uint8 u8sns_cmplt = 0;

static tsSnsObj sSnsObj;
static tsObjData_MLX90614 sObjMLX90614;

enum {
	E_SNS_ADC_CMP_MASK = 1,
	E_SNS_MLX90614_CMP = 2,
	E_SNS_ALL_CMP = 3
};
...
PRSEV_HANDLER_DEF(E_STATE_IDLE, tsEvent *pEv, teEvent eEvent, uint32 u32evarg) {
	if (eEvent == E_EVENT_START_UP) {
		...
		// MLX90614 の初期化
		vMLX90614_Init(&sObjMLX90614, &sSnsObj);
		vSnsObj_Process(&sSnsObj, E_ORDER_KICK);
		if (bSnsObj_isComplete(&sSnsObj)) {
			// 即座に完了した時はセンサーが接続されていない、通信エラー等
			u8sns_cmplt |= E_SNS_MLX90614_CMP;
			V_PRINTF(LB "*** MLX90614 comm err?");
...
PRSEV_HANDLER_DEF(E_STATE_RUNNING, tsEvent *pEv, teEvent eEvent, uint32 u32evarg) {
	...
	if ((eEvent == E_EVENT_START_UP) && (u32evarg & EVARG_START_UP_WAKEUP_RAMHOLD_MASK)) {
		V_PRINTF("#");
		vProcessMLX90614(E_EVENT_START_UP);
	}
	...
	if (u8sns_cmplt != E_SNS_ALL_CMP && (u8sns_cmplt & E_SNS_ADC_CMP_MASK)) {
		...
		//
		// #Mikimasa センサーの変換時間が50msより長い場合は以下の関数のスリープ時間(MLX90614_WAITTIME)を
		// #必要な待ち時間（変換時間）に変える必要がある。(200msなど。元ソースでは50ms)
		// #なぜなら、このスリープから起床すると強制的に読み出しが実行されるため。
		//ToCoNet_vSleep(E_AHI_WAKE_TIMER_1, 50, FALSE, FALSE); // PERIODIC RAM OFF SLEEP USING WK1
		ToCoNet_vSleep(E_AHI_WAKE_TIMER_1, MLX90614_WAITTIME, FALSE, FALSE); // PERIODIC RAM OFF SLEEP USING WK1
	}
...
PRSEV_HANDLER_DEF(E_STATE_APP_WAIT_TX, tsEvent *pEv, teEvent eEvent, uint32 u32evarg) {
	if (eEvent == E_EVENT_NEW_STATE) {
		...
		S_BE_WORD(sObjMLX90614.i16Result);
...
static void cbAppToCoNet_vHwEvent(uint32 u32DeviceId, uint32 u32ItemBitmap) {
	switch (u32DeviceId) {
	case E_AHI_DEVICE_TICK_TIMER:
		vProcessMLX90614(E_EVENT_TICK_TIMER);
		break;
...
static void vProcessMLX90614(teEvent eEvent) {
	...
	if (bSnsObj_isComplete(&sSnsObj)) {
		...
		V_PRINTF(LB"!MLX90614: %d.%02dC",
			sObjMLX90614.i16Result / 100, sObjMLX90614.i16Result % 100
		);
